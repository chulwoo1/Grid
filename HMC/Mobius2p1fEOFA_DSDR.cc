...
//  HMCparams.MD = MD;
  ...
  const int Ls      = 24;
  Real beta         = 1.633;
  Real light_mass   = 0.00107;
  Real strange_mass = 0.0850;
  Real pv_mass      = 1.0;
  RealD M5  = 1.8;
  RealD b   = 2.5; 
  RealD c   = 1.5;

  std::vector<Real> hasenbusch({ 0.00789, 0.0291, 0.095,0.3,0.548 });
  ...
  for(int h=0;h<light_num.size();h++){
...
// DSDR
  ...
  typedef typename TMActionF::FermionField TMFieldF;
  std::vector<TMAction *> NumTM;
  std::vector<TMAction *> DenTM;
//  std::vector<TwoFlavourEvenOddRatioPseudoFermionAction<FermionImplPolicy> *> QuoTM;

  for(int h=0;h<1;h++){

    std::cout << GridLogMessage << " 2f DSDR Action  "<< light_num[h] << " / " << light_den[h]<< std::endl;

    NumTM.push_back(new TMAction(U,*GridPtr,*GridRBPtr,-M5,light_num[h],Params));
    DenTM.push_back(new TMAction(U,*GridPtr,*GridRBPtr,-M5,light_den[h],Params));

    ////////////////////////////////////////////////////////////////////////////
    // Standard CG for 2f force
    ////////////////////////////////////////////////////////////////////////////
    Level2.push_back   (new TwoFlavourEvenOddRatioPseudoFermionAction<FermionImplPolicy>(*NumTM[h],*DenTM[h],DerivativeCG,ActionCG));
