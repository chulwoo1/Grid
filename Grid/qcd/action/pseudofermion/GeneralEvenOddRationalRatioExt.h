    ...
    *************************************************************************************/
    /*  END LEGAL */
#ifndef QCD_PSEUDOFERMION_GENERAL_EVEN_ODD_RATIONAL_RATIO_EXT_H
#define QCD_PSEUDOFERMION_GENERAL_EVEN_ODD_RATIONAL_RATIO_EXT_H
...
NAMESPACE_BEGIN(Grid);
    ...
    */
    ...
    template<class Impl>
    class GeneralEvenOddRatioRationalPseudoFermionActionExt : public Action<typename Impl::GaugeField> {
    public:
    ...
    private:
      ...
      static void generateApprox(MultiShiftFunction &approx, MultiShiftFunction &approx_inv, int inv_pow, int approx_degree, double CG_tolerance, AlgRemez &remez, RealD *shift_=NULL){
	std::cout<< "Generating degree "<< approx_degree<<" approximation for x^(1/" << inv_pow << ")"<<std::endl;
	if(shift_)  std::cout<< "shift= "<< *shift_ << std::endl;
//	exit(-42);
	if (inv_pow ==1){
//  1/(x+shift)
		int order=1;
		approx.norm=0.;
		approx.order=order;
		approx.tolerances.resize(order);
		approx.tolerances[0]=CG_tolerance;
		approx.poles.resize(order);
		approx.poles[0]=0.;
		if(shift_) approx.poles[0]=*shift_;
		approx.residues.resize(order);
		approx.residues[0]=1.;
		approx.poly.resize(0);

//  x+shift !!
		approx_inv.order=0;
		approx_inv.tolerances.resize(0);
		approx_inv.poles.resize(0);
		approx_inv.residues.resize(0);
		approx_inv.poly.resize(1);
		approx_inv.poly[0]=1.;
		approx_inv.norm=0.;
		if(shift_) approx_inv.norm=*shift_;

	} else {
	double error = remez.generateApprox(approx_degree,1,inv_pow,shift_);	
	...
	ConjugateGradientMultiShift<FermionField> msCG(MaxIter, approx);
        std::cout<<GridLogMessage << "multiShiftInverse: approx.order= "<<approx.order<<" poly "<<approx.poly.size()<<std::endl;
	if (approx.order>0)
	  msCG(schurOp,in, out);
	else
	  out = approx.norm*in;
	 FermionField tmp(in.Grid());
	 FermionField tmp2(in.Grid());
	 tmp=in;
	 for (int i=0;i<approx.poly.size();i++){
	    schurOp.HermOp(tmp,tmp2);
	    tmp=tmp2;
	    out += approx.poly[i]*tmp;
	 }
...
//	assert(    md_tolerance.size()==ApproxPowerMD.tolerances.size());
	...
	for(int i=0;i<ApproxNegPowerAction.tolerances.size();i++)
	  ApproxNegPowerAction.tolerances[i]    = action_tolerance[i];
	for(int i=0;i<ApproxHalfPowerAction.tolerances.size();i++)
	  ApproxHalfPowerAction.tolerances[i]   = action_tolerance[i];
	for(int i=0;i<ApproxNegHalfPowerAction.tolerances.size();i++)
	...
	*/
      ...
      GeneralEvenOddRatioRationalPseudoFermionActionExt(FermionOperator<Impl>  &_NumOp, 
	...
	generateApprox(ApproxPowerAction, ApproxNegPowerAction, param.inv_pow, param.action_degree, param.action_tolerance, remez,&(param.DenShift));
	generateApprox(ApproxHalfPowerAction, ApproxNegHalfPowerAction, 2*param.inv_pow, param.action_degree, param.action_tolerance, remez,&(param.NumShift));
	...
	if(param.md_degree != param.action_degree){ //note the CG tolerance is unrelated to the stopping condition of the Remez algorithm
	  ...
	  std::cout<<GridLogMessage << "Ext: Using same rational approximations for MD as for action evaluation" << std::endl;
	  ApproxPowerMD = ApproxPowerAction; 
	  ApproxNegPowerMD = ApproxNegPowerAction;
	  std::cout<<GridLogMessage << "Ext: ApproxNegPowerMD "<<ApproxNegPowerMD.tolerances.size()<< std::endl;
	  for(int i=0;i<ApproxNegPowerMD.tolerances.size();i++)
	    ApproxNegPowerMD.tolerances[i] = param.md_tolerance; //used for multishift
	  std::cout<<GridLogMessage << "Ext: ApproxPowerMD "<<ApproxPowerMD.tolerances.size()<< std::endl;
	  for(int i=0;i<ApproxPowerMD.tolerances.size();i++)
	    ApproxPowerMD.tolerances[i] = param.md_tolerance; //used for multishift
	  

	  ApproxHalfPowerMD = ApproxHalfPowerAction;
	  ApproxNegHalfPowerMD = ApproxNegHalfPowerAction;
	  std::cout<<GridLogMessage << "Ext: ApproxNegHalfPowerMD "<<ApproxNegHalfPowerMD.tolerances.size()<< std::endl;
	  for(int i=0;i<ApproxNegHalfPowerMD.tolerances.size();i++)
	    ApproxNegHalfPowerMD.tolerances[i] = param.md_tolerance;
	  std::cout<<GridLogMessage << "Ext: ApproxHalfPowerMD "<<ApproxHalfPowerMD.tolerances.size()<< std::endl;
	  for(int i=0;i<ApproxHalfPowerMD.tolerances.size();i++)
	     ApproxHalfPowerMD.tolerances[i] = param.md_tolerance;
	}
	...
	std::cout<<GridLogMessage << action_name() << " initialize: complete" << std::endl;
      ...
      virtual std::string action_name(){return "GeneralEvenOddRatioRationalPseudoFermionActionExt";}
